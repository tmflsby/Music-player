...
<template>
  <div class="list-item">
    ...
    <div class="img-info" :class="{ bigImg: videoList, smallImg: songList, circle, album, dj }">
      <!-- 视频的播放量 -->
    ...
    <div class="info-content">
      ...
      <div class="play-name">
        {{name}}
        <!-- 在用户区域用类名显示不同的用户性别图标 -->
        <i class="result" :class="{ nan: gender === 1, nv: gender === 2}" v-if="gender"></i>
      </div>
      ...
      <div class="play-tag">
        ...
        <p class="video" v-if="videoList">
          <span class="time">
            {{durationms | setTime}}
            <span>by
              <span class="video-art" v-for="(item, index) in nicknames" :key="index">
                {{item.userName}}
              </span>
            </span>
          </span>
